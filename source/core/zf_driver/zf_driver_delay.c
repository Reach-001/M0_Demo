...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
/* BUG FIX 4（潜在）: 原代码 time 为 uint32，当 time=0 时 while(time--) 先判断（非0为假）
 * 再递减，uint32 下溢到 0xFFFFFFFF，进入约49天死循环。
 * 在函数入口添加 if(0 == time) return 守卫，避免下溢。 */
void system_delay_ms (uint32 time)
{
    if (0 == time) { return; }  /* 守卫：防止 uint32 下溢死循环 */
